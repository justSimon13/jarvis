...
**Seit Thread-Umbau Teil A (2026-08-01):** Threads sind für jarvis-web das EINZIGE Modell — "Kein Thema" entfällt, jede Nachricht bekommt einen Thread. Der Server erzwingt das aber bewusst NICHT strukturell: `build_history_window(thread_id=None)`s Cursor-Fallback bleibt vollständig bestehen, weil `category="voice"` kein Thread-Konzept hat und `category="web"` laut `_category_for_role()` allein über `role="dashboard"` vergeben wird, nicht exklusiv jarvis-web (ein mögliches jarvis-dashboard könnte sich denselben Client-Typ teilen, ohne je `thread_id` zu senden). Stattdessen sorgt jarvis-web selbst per Bootstrap dafür, dass für seine Verbindungen immer ein Thread aktiv ist: nach jedem `requestThreads()` wird bei fehlendem oder ungültig gewordenem `activeThreadId` automatisch ein neuer, unbenannter Thread angelegt (`stores/jarvis.js::newThread()`). Nachrichten lassen sich nachträglich zwischen Threads verschieben (`move_round`/`move_from_here`, immer ganze, in sich abgeschlossene Runden — siehe `session_memory.py`, CODE_REFERENCE.md) und Threads zusammenführen (`merge_threads`). Details, Protokolltypen und Backend-Funktionen: `ROADMAP.md` (Eintrag "Thread-Umbau Teil A"), `CODE_REFERENCE.md`.

**Seit Thread-Umbau Teil B, Schritt 1 (2026-08-01):** ein unbenannter Thread (`title IS NULL`) bekommt automatisch einen inhaltsbeschreibenden Kurztitel — neues Top-Level-Modul `thread_naming.py`, angestoßen aus `pipeline.py::_run_llm()` nach jedem echten Rundenabschluss (nicht bei Tool-Use-Zwischenschritten), läuft in einem eigenen Hintergrund-Thread und blockiert die bereits emittierte Chat-Antwort nicht. Nur bei Rundenstand 2, 4 oder 8 wird ein billiger, nicht-streamender Haiku-4.5-Call (`llm.complete()`, Gegenstück zu `stream()` ohne dessen Streaming-/Cache-Control-/24k-Token-Overhead) versucht; ein bestehender manuell vergebener Titel wird nie überschrieben (`session_memory.set_auto_title()`, atomares `UPDATE ... WHERE title IS NULL`). Ein zusätzlicher, einmaliger Startup-Sweep (`thread_naming.run_startup_sweep()`, aus `server.py::main()`) benennt bereits bestehende unbenannte Threads nach, in denen längst nicht mehr geschrieben wird — der Live-Hook erreicht die sonst nie. Ergebnis geht per neuem `THREAD_TITLE_UPDATED`-Push an ALLE verbundenen Web-Clients (nicht nur den auslösenden Tab, anders als `THREAD_REASSIGNED` — reine Anzeige-Aktualisierung ohne Zustandsrisiko), über `server.py::_broadcast_web_event()`, die `pipeline.py` per Dependency Injection (`JarvisPipeline(..., broadcast_web_event=...)`) erreicht, da ein direkter Import von `server.py` aus `pipeline.py` zirkulär wäre. Details: `ROADMAP.md` (Eintrag "Thread-Umbau Teil B, Schritt 1"), `CODE_REFERENCE.md`.
