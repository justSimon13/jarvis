...
Fix: `_is_safe_readonly_command()` — eine enge, bewusst konservative Whitelist (`ls`/`cat`/`pwd`/`head`/`tail`/`wc`/`stat`/`file`/`which`/`df`/`du`/`ps`/`whoami`/`uname`/`date`/`uptime`/`free`/`hostname`, dazu `systemctl status`/`is-active`/`list-*`, `git status`/`log`/`diff`/`show`/`branch`/`remote`, `journalctl` ohne Vacuum/Rotate-Flags — explizit NICHT `find`, wegen `-delete`/`-exec`, und NICHT `curl`/`wget`, schon anderweitig als riskant geflaggt). Kein Chaining erlaubt (`&&`/`;`/`|`/Umleitung/Sub-Shell blockieren den Fastpath). Nur Befehle die exakt matchen laufen SOFORT synchron in `run_command()` selbst, Ergebnis kommt direkt als Tool-Antwort zurück — JARVIS sieht Erfolg/Fehler noch in derselben Antwort und kann selbst nachjustieren (z.B. bei falschem Pfad `ls ~/` nachschieben um zu sehen was es wirklich gibt). Alles andere läuft weiterhin über den bestehenden Freigabe+Notification-Weg — Tool-Beschreibung sagt jetzt explizit, dass dort NICHT selbst nachgeprüft werden kann und das Simon so mitgeteilt werden soll. Lokal mit 17 Testfällen gegen die Whitelist-Logik verifiziert (inkl. aller Umgehungsversuche: Chaining, Pipes, Umleitung, sudo, Force-Push, `find -delete`, `curl | sh`), plus End-to-End-Test der exakten Fehlerszenerie aus dem Screenshot.
