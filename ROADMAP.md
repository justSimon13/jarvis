...
**Bekannte Grenze:** `sudo`-Befehle funktionieren nur mit einer vorher eingerichteten NOPASSWD-sudoers-Regel für genau diesen Befehl (gleiches Muster wie `install_auto_update.sh` für `systemctl restart jarvis.service`) — ohne das hängt der Befehl in einem nie beantworteten Passwort-Prompt bis zum Timeout. Bewusst keine pauschale sudo-Freigabe eingerichtet (wäre praktisch Root-Zugriff für JARVIS). Für oft gebrauchte privilegierte Befehle (z.B. `sudo systemctl restart jarvis-web`) müsste Simon bei Bedarf eine eigene schmale sudoers-Regel anlegen, analog zum Auto-Update-Timer.
