...
- PR #18: **eigentliche Ursache gefunden** (durch Simons eigenen Shell-vs-Finder-Start-Vergleich) — Rusts `Command` löst einen bloßen Programmnamen über `execvp()` mit dem PATH des ELTERNPROZESSES (dieser App) auf, nicht mit dem per `env()` gesetzten PATH des Kindprozesses (wirkt erst innerhalb des Kindprozesses). Der bisherige `env.PATH`-Override konnte `claude`/`gh` deshalb strukturell nie aus einer per Finder gestarteten App (nur minimaler `launchd`-PATH geerbt) auffindbar machen. Fix: `std::env::set_var("PATH", ...)` beim App-Start (`extend_app_path()`, `src-tauri/src/lib.rs`) erweitert das PATH des App-Prozesses SELBST, bevor irgendein `Command` läuft — der `env.PATH`-Override beim Spawn bleibt zusätzlich bestehen (wirkt für Unterprozesse von `claude` selbst).
- **PR #19 (`jarvis-web`) + PR #16 (`jarvis` server, dieser Lauf):** Fehler blieb laut Simon auch nach PR #18 generisch — vollständiger Audit von `local_exec_request` bis zum Spawn fand fünf weitere echte ungefangene Stellen (fehlendes `.catch()` in `jarvis.js`, ungeschütztes `JSON.parse` in `runGhIssueList`, ungeschützter Schreibvorgang in `_writeAllowlistPaths`, ungeschützte Listener-Registrierung + fire-and-forget-Aufruf in `_prepareAndRunJob`/`runClaudeCodeRun`), alle geschlossen. Zusätzlich neues Diagnose-Tool: `diagnose_coding_worker` (Server) → `diagnose_binaries`-Aktion (Worker) liefert ohne Job-Start das PATH des App-Prozesses (neuer `get_env_path`-Rust-Command), das zusammengesetzte Subprozess-PATH, den `allowlist.json`-Inhalt sowie Existenz+`--version`-Probelauf pro Binary — damit sich der nächste Verdacht direkt prüfen lässt statt bei jedem Fehlschlag blind neu zu bauen.

**Noch offen:** beide PRs (#19 `jarvis-web`, #16 `jarvis`) sind noch nicht gemerged/deployed — Simon muss nach dem nächsten Release `diagnose_coding_worker` im Chat aufrufen, um den tatsächlichen Live-Zustand zu sehen. Release-Pipeline (`tauri-release.yml`) war zuletzt (`v0.1.11`/`v0.1.12`) wegen eines GitHub-Billing-Problems fehlgeschlagen (nicht code-seitig lösbar) — letzter erfolgreicher Release war `v0.1.10`; unklar ob seither getestete Builds tatsächlich alle Fixes enthielten.
