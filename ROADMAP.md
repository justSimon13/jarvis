...
- **Empirisch widerlegt:** ein schmales `--allowedTools 'Bash(gh issue list:*)'` überschreibt KEIN breiteres `--disallowedTools 'Bash(gh:*)'` — echter `claude -p`-Testlauf (Scratch-Repo, Fake-`gh`-Binary) verweigerte den Aufruf trotzdem ("Permission ... has been denied"). Deny gewinnt unabhängig von der Spezifität, nicht wie ursprünglich angenommen. Deshalb umgekehrt gelöst: `_claudeToolFlags()` in `localExec.js` lässt `Bash` weiterhin generell zu und disallowed stattdessen einzeln aufgezählte gh-Schreibunterbefehle (`gh pr create/edit/merge/close/reopen/review`, `gh issue create/comment/close/edit/reopen/delete`, `gh repo delete/edit`, `gh release`, `gh workflow run`, `gh label`, `gh secret`, `gh variable`, plus `gh api` als universeller Schreib-Fluchtweg) — Lesen (`gh issue list`/`view`) bleibt über das generelle Bash-Allow möglich. Kein vollständiger Blocklist-Anspruch (z.B. seltene `gh`-Unterbefehle wie `gh gist`/`gh org` nicht einzeln aufgeführt, für den Coding-Job-Anwendungsfall nicht relevant).
- **`projekte.issue_repo`** (optional): getrenntes Ticket-Repo — bei Simon liegen Issues aller Code-Repos in einem eigenen Repo. Gesetzt, läuft `gh issue view`/`list` für Coding-Jobs gegen `issue_repo` statt `repo` (Substitution serverseitig in `coding_jobs.py::_try_dispatch()`, reiner Feld-Swap vor dem Dispatch an den Worker — `cwd` bleibt unverändert das Code-Repo). `gh pr create` unberührt (arbeitet ohnehin ohne `--repo` auf dem Git-Remote von `cwd`).
- **`projekte.delivery`** (`local`/`push`/`pr`, Default `pr` = bisheriges Verhalten): unabhängig von `autonomy` — zwei getrennte Achsen (Kontrolle VOR der Ausführung vs. wie weit das Ergebnis geht). `local`: Commit auf dem Job-Branch, kein Push, kein PR, Checkout danach zurück auf `base_branch` (bestehender Code-Pfad unverändert), Branch bleibt bestehen (wird nirgends gelöscht außer bei `discard_job`). Strukturell durchgesetzt, nicht nur im Ablauf weggelassen: `git push` ist dem Modell in JEDEM Modus gesperrt (unverändert), `gh` bei `delivery='local'` zusätzlich KOMPLETT gesperrt (nicht nur die Schreibunterbefehle wie sonst) — siehe `_claudeToolFlags(readOnly, delivery)`. `push`: Commit + Push, kein PR. Snapshot bei `start_job()` in die `jobs`-Tabelle (wie `autonomy`/`cwd`/`base_branch`) — eine spätere Änderung von `projekte.delivery` betrifft nie einen schon laufenden/wartenden Job, auch nicht über eine `approve_coding_job`-Fortsetzung.
