...
- **Nicht abschließend geklärt, ob das TATSÄCHLICH die Ursache in diesem konkreten Fall war** (kein Zugriff auf die echten Server-Logs von hier aus) — zur Bestätigung: `journalctl -u jarvis.service` um den Neustart-Zeitpunkt prüfen, ob die Zeile `"[server] Laufende Sessions vor Shutdown gesichert."` erscheint. Fehlt sie, war es kein sauberer, per SIGTERM eingeleiteter Neustart (`_save_all_sessions_on_shutdown()` lief nie) — das würde eher auf einen Absturz oder ein hartes Kill hindeuten (der jetzige Fix hilft dann nur bedingt, das eigentliche Problem wäre dann WARUM der Prozess ohne sauberes Shutdown-Signal endete). Erscheint die Zeile, bestätigt das den oben beschriebenen fire-and-forget-Race als tatsächliche Ursache.

Verifiziert: echter Import von `server.py` (volle Dependency-Kette, u.a. `websockets`) in einem Scratch-Venv, `_persist_web_turn()` als Coroutine-Funktion bestätigt, gemockter `session_memory.upsert()` beweist: nach dem `await` ist der Write garantiert bereits gelaufen, `_active_session_ids` korrekt aktualisiert. `py_compile` sauber.
