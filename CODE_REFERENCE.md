...
- `__init__` (Z. 66) — nimmt `shared_history` (dasselbe Listenobjekt wie `server.py`s `api_histories`), gemeinsamen `history_lock`, gemeinsamen `llm_semaphore`. Trackt `_executed_tool_ids` (Dedup) und `_active_modules`.
...
- `_run_llm()` (Z. 233) — Tool-Call-Loop: streamt via `llm.stream()`, puffert Text satzweise für TTS (`SENTENCE_END`-Regex, Z. 25, `TTS_BUFFER_MIN=120`), führt bei `stop_reason=="tool_use"` jeden Block via `tools.execute()` aus (Z. 338), Dedup gegen `_executed_tool_ids` pro `call_id`, komprimiert Tool-History jede Iteration (Z. 346), loopt bis `end_turn`.
