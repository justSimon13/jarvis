...
**Konvention `topic="finanzen", key="gewinn"`** (seit 2026-07-26) — realisierte Gewinne, Gegenstück zu `projekte.geschaetzter_wert` (`local_data.py`). `server.py`s `finanzen_overview`-Resource kombiniert beides. `"finanzen"` ist deshalb aus dem generischen `tracking_topics`-Ergebnis ausgeschlossen (wie `"coding_engine"` und seit 2026-07-27 auch `"chat"` — `pipeline.py`s `cost_usd`-Logs, dieselbe Quelle wie `store.chatCostToday`) — LLM-Verbrauchskosten haben mit jarvis-web's eigener "Kosten"-Seite (`KostenView.vue`) eine dedizierte Ansicht, getrennt von den allgemeinen Lebens-Statistiken (Sport etc.) auf der "Tracking"-Seite.
