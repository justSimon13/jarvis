...
- `SESSION_RESET`/`SESSION_LOAD` — seit 2026-07-31 zusätzlich `session_memory.advance_cursor()`/`rewind_cursor_to_session()` statt nur die In-Memory-Liste zu leeren/ersetzen. `SESSION_LOAD` prüft vorher `session_memory.session_belongs_to_tab()` — laden einer Session eines FREMDEN `tab_id` liefert jetzt `ERROR` statt stiller Cross-Tab-Übernahme (Cross-Tab-Fortsetzung ist ein künftiges Thread-Feature). `SESSION_RESET` setzt seit Teil 2 zusätzlich `_active_thread_ids[tab_id]=None`/`pipeline.set_thread(None)` — Handler unverändert, wird von jarvis-web seit Thread-Umbau Teil A aber nicht mehr gesendet ("+ Neuer Chat" legt jetzt einen Thread an, siehe `stores/jarvis.js::newThread()`), bleibt für andere `role="dashboard"`-Clients (z.B. jarvis-dashboard) bestehen.
- `SET_THREAD`-Handler (Teil 2) — `{"type": "set_thread", "thread_id": N|null}`. Lookup von `project_id` zum Thread via `run_in_executor` (bewusst NICHT synchron wie `SET_MODE`/`SET_THINKING`, die brauchen keine DB), rückt danach IMMER `session_memory.advance_cursor(category, tab_id)` vor (Thread-Nachrichten teilen sich denselben `tab_id` wie threadlose — ohne diesen Vorstoß würde das Cursor-Fenster sie beim Zurückwechseln im Cursor-Modus wieder mit anzeigen), setzt `_active_thread_ids[tab_id]` und `pipeline.set_thread(...)`. `thread_id: null` bleibt serverseitig gültig (Cursor-Fallback, s.o.) — nur jarvis-web sendet es seit Teil A nie mehr.
- `MOVE_MESSAGES`-Handler (Thread-Umbau Teil A, neu) — `{"type":"move_messages","message_id":N,"mode":"from_here"|"single_round","target_thread_id":N}`. Wählt `session_memory.move_round` (bei `"single_round"`) oder `move_from_here` (Standard) via `run_in_executor`, antwortet `MOVE_MESSAGES_ACK` (`{ok, error?}` — `error` z.B. wenn `message_id` kein gültiger, abgeschlossener Rundenanfang ist).
- `MERGE_THREADS`-Handler (Thread-Umbau Teil A, neu) — `{"type":"merge_threads","source_thread_id":N,"target_thread_id":N}`. Ruft `session_memory.merge_threads()` via `run_in_executor`. Bei Erfolg: iteriert über ALLE `_active_thread_ids` (auch den auslösenden Tab selbst, falls dessen aktiver Thread der Quell-Thread war) — für jeden betroffenen Tab wird `_active_thread_ids[tab_id]` umgehängt, `manager.get_pipeline_for_tab(tab_id).set_thread(...)` aktualisiert die LIVE-Pipeline, und `THREAD_REASSIGNED` (`{old_thread_id, new_thread_id}`) geht an dessen Event-Callback (`manager.get_event_callback_for_tab()`) — sonst würde die nächste dort geschriebene Nachricht unter einer inzwischen gelöschten `thread_id` landen. Antwort: `MERGE_THREADS_ACK` (`{ok, error?}`).
- `_deliver_job_result_to_chat()` — seit 2026-07-31 zusätzlich `session_memory.append_message(..., display_text=...)` (kurze Version an die API, volle an die UI). Seit Teil 2: schlägt zusätzlich `_active_thread_ids.get(tab_id)` nach und reicht `thread_id`/`project_id` durch — läuft außerhalb jeder Pipeline-Runde (der Job kann Minuten/Stunden vorher gestartet worden sein), ohne das würde ein Ergebnis während eines aktiven Threads außerhalb von dessen Fenster landen. Seit Thread-Umbau Teil A: nutzt `session_memory.append_or_extend_message()` statt eines direkten `append_message()`-Inserts — verhindert zwei aufeinanderfolgende `assistant`-Zeilen, falls seit dem Job-Start keine weitere Nachricht im Ziel-Fenster geschrieben wurde (der Normalfall: Job starten, warten).
- `_broadcast_web_event(event)` (Thread-Umbau Teil B, Schritt 1, neu, neben `manager = ClientManager()`) — schickt `event` an ALLE über `manager.get_dashboard_event_callbacks()` registrierten Web-/Dashboard-Clients, nicht nur einen. Wird an `JarvisPipeline(...)` als `broadcast_web_event`-Parameter durchgereicht (Dependency Injection — `pipeline.py` kann `server.py`s `manager`-Singleton nicht direkt importieren, zirkulär) UND an `thread_naming.run_startup_sweep()` beim Server-Start übergeben. Erster/einziger Aufrufer aktuell: `thread_naming.py`.
- `_save_all_sessions_on_shutdown()`, `main()` — Startup: `session_memory.migrate_sessions_to_messages()` (einmalig, idempotent), `brain.sync()`, `knowledge.rebuild_index()`, History laden, STT-Modell laden, alle `services/*`-Singletons initialisieren. Seit Thread-Umbau Teil B, Schritt 1 zusätzlich: `threading.Thread(target=thread_naming.run_startup_sweep, args=(_broadcast_web_event,), daemon=True).start()` — einmaliger Nachhol-Durchlauf für bereits bestehende unbenannte Threads, eigener Hintergrund-Thread, blockiert den Start nicht. Shutdown archiviert offene Sessions (alte `sessions`-Tabelle).
...
- `__init__` (Z. 66) — nimmt `shared_history` (dasselbe Listenobjekt wie `server.py`s `api_histories`), gemeinsamen `history_lock`, gemeinsamen `llm_semaphore`. Trackt `_executed_tool_ids` (Dedup) und `_active_modules`. Seit 2026-07-25 zusätzlich `_thinking_enabled: bool` (Default `False`) und `_model: str` (Default `llm.MODEL`) — pro Client/Session einstellbar, nicht global. Seit Thread-Umbau Teil B, Schritt 1: optionaler Parameter `broadcast_web_event` (Default `None`), gespeichert als `self._broadcast_web_event` — Dependency Injection für die automatische Thread-Benennung (siehe `_run_llm()` unten und `thread_naming.py`), da `pipeline.py` `server.py`s `manager`-Singleton nicht direkt importieren kann (zirkulär). `main.py`s Voice-Only-Entrypoint lässt den Parameter weg, harmlos da Voice-Pipelines nie `self._thread_id` setzen.
...
**Seit Thread-Umbau Teil B, Schritt 1:** in `_run_llm()`, direkt nach `self._emit(P.RESPONSE_DONE, ...)` und noch vor dem `break` im `end_turn`/`refusal`/`max_tokens`-Zweig — falls `self._thread_id`, `assistant_message_id` UND `self._broadcast_web_event` alle gesetzt sind (echter Rundenabschluss, nicht ein `tool_use`-Zwischenschritt, und ein Web-Kontext mit Broadcast-Kanal), wird `threading.Thread(target=thread_naming.maybe_name_thread_on_turn, args=(self._thread_id, self._broadcast_web_event), daemon=True).start()` gefeuert — läuft im Hintergrund, blockiert die bereits emittierte Antwort nicht. Die Rundenzahl-Prüfung (nur bei Runde 2/4/8) passiert innerhalb dieses Hintergrund-Threads, nicht hier (siehe `thread_naming.py`).

---

## thread_naming.py (~130 Zeilen, neu, Thread-Umbau Teil B, Schritt 1) — automatische Thread-Benennung

Top-Level-Modul (nicht unter `services/` — Turn-Abschluss-Logik wie `context.py`, kein isolierter Service, muss direkt aus `pipeline.py` UND `server.py` importierbar sein).

- `_attempt_naming(thread_id, broadcast)` — Kernfunktion, keine eigene Rundenzahl-Prüfung (liegt bei den beiden Aufrufern unten). Ablauf: `session_memory.get_thread_title()`-Sicherheitsnetz (falls der Titel zwischen Aufrufer-Check und hier gesetzt wurde) → Prompt aus `session_memory.list_named_titles(limit=300)` + `session_memory.get_thread_messages(thread_id)` bauen (Verlauf auf ~4000 Zeichen gekappt, nur ganze Zeilen, nie mitten in einer Nachricht abgeschnitten) → `llm.complete(_SYSTEM_PROMPT, ..., model="claude-haiku-4-5", max_tokens=60)` → Antwort `"UNCLEAR"`/leer → kein Schreiben; sonst bereinigen (Anführungszeichen/Whitespace strippen, auf 60 Zeichen kappen) und `session_memory.set_auto_title()` — nur bei `True` (tatsächlich geschrieben, kein Race verloren) `broadcast({"type": P.THREAD_TITLE_UPDATED, "thread_id": ..., "title": ...})`. Komplett in try/except, Fehler mit Prefix `[thread_naming]` geloggt, propagiert nie aus dem Hintergrund-Thread.
- `maybe_name_thread_on_turn(thread_id, broadcast)` — Live-Hook-Einstiegspunkt (`pipeline.py`). Prüft zuerst `get_thread_title()` (billiger Vorab-Check, spart die Rundenzählung bei einem längst benannten Thread im Normalfall), dann `session_memory.count_rounds(thread_id) in (2, 4, 8)` — nur bei EXAKT diesen drei Rundenständen wird `_attempt_naming()` aufgerufen. Da jeder abgeschlossene Turn die Rundenzahl um genau 1 erhöht, trifft ein aktiver Thread jeden der drei Stände natürlicherweise genau einmal — max. drei LLM-Aufrufe pro Thread über die gesamte Lebensdauer im laufenden Betrieb.
- `maybe_name_thread_once(thread_id, broadcast)` — Startup-Sweep-Einstiegspunkt (`server.py::main()`), pro Thread genau ein Aufruf. Nur `count_rounds(thread_id) >= 2`, keine Obergrenze wie beim Live-Hook (hier wird ohnehin nur einmal versucht, nicht wiederholt).
- `run_startup_sweep(broadcast)` — einmaliger Durchlauf beim Serverstart über `session_memory.list_unnamed_thread_ids()`, ruft für jeden Treffer NACHEINANDER `maybe_name_thread_once()` auf mit 3s-Pause dazwischen (`_SWEEP_THROTTLE_SECONDS`, gedrosselt — kein Burst gleichzeitiger API-Calls). Läuft in einem eigenen Hintergrund-Thread (siehe `server.py::main()`), bewusst OHNE gemeinsames Rate-Limiting mit dem `llm_semaphore` der Chat-Pipeline — ein langsamer Benennungs-Call darf nie einen echten Chat blockieren und umgekehrt, `llm.complete()`s eigener 120s-Timeout bleibt die einzige Absicherung. Läuft bei jedem Neustart erneut über die dann noch verbleibenden unbenannten Threads (akzeptierter Kompromiss statt einer "nur einmal jemals"-Sperre).
- `_SYSTEM_PROMPT` — deutsch, weist das Modell an: 2-4 Wörter, wie ein Aktenordner-Etikett, bei erkennbar gleichem Thema GENAU den mitgelieferten bestehenden Titel wiederverwenden (statt eine Variante zu erfinden — verhindert inkonsistente Duplikate wie "Steuern"/"Steuererklärung"/"Finanzamt"), sonst exakt `UNCLEAR` bei noch unklarem Thema.
...
- `get_event_callback_for_tab(tab_id)`/`get_pipeline_for_tab(tab_id)` (Thread-Umbau Teil A) — Tab-gezielter Zugriff (anders als `get_active()`, das den zuletzt aktiven Client meint). `get_pipeline_for_tab()` neu seit Teil A: `MERGE_THREADS`-Handler (`server.py`) braucht sie, um bei einem betroffenen, gerade verbundenen Tab dessen LIVE-Pipeline-Instanz auf den neuen Ziel-Thread umzuhängen (`pipeline.set_thread(...)`), nicht nur eine Client-Benachrichtigung zu schicken.
...
**Neu, die eigentliche Prompt-Quelle:** Tabellen `messages` (`role` NUR `user`/`assistant`, `content` JSON-encoded volle API-Fidelity, `display_text` nullable für UI-Abweichungen, `attachments`/`client_name` für UI, `category`/`tab_id` als bewusste Zwischenlösung fürs Fenstern, `thread_id`/`project_id` seit Teil 2 befüllt (seit Thread-Umbau Teil A für jarvis-web IMMER gesetzt, serverseitig aber weiterhin optional — siehe unten), `data_scope` Default `'own'`), `history_windows` (Cursor `active_after_id` pro `(category, tab_id)` — Ersatz fürs Leeren einer Liste, echter Append-Only-Strom, bleibt bestehen: Voice hat kein Thread-Konzept, und `category="web"` ist laut `_category_for_role()` nicht exklusiv jarvis-web), `threads` (`id, title, project_id, last_activity_at, created_at, summary, data_scope` — `title` seit Teil A nullable, `created_at` seit Teil A neu (einmalig bei Anlage gesetzt, ändert sich anders als `last_activity_at` nie wieder — Grundlage für die "Unbenannt · HH:MM"-Anzeige in der Sidebar), `summary` bleibt unbenutzt), `daily_summaries` (nur Schema, unbenutzt).
...
- `append_or_extend_message(...)` (Thread-Umbau Teil A) — für `server.py::_deliver_job_result_to_chat()`: ist die letzte Zeile des Ziel-Fensters bereits `assistant` mit reinem String-`content`, wird der Text ANGEHÄNGT (`UPDATE`, `content`+`display_text` erweitert) statt eine neue Zeile zu schreiben — sonst könnten zwei aufeinanderfolgende `assistant`-Zeilen entstehen (Job gestartet, keine weitere Nachricht bis zur Fertigstellung ist der Normalfall, nicht der Randfall — sonst würde das Ergebnis praktisch nie im Kontext landen). Ist die letzte Zeile `user` oder das Fenster leer, verhält sich die Funktion wie ein normaler `append_message()`-Aufruf. Zweite, eng begrenzte Ausnahme vom "Nachrichten werden nie mutiert"-Prinzip (nach dem Turn-Rollback aus Teil 1) — bewusst in Kauf genommen, weil eine neue Zeile die Rollen-Abwechslung bräche. Bleibt mit der Rundenerkennung (s.u.) verträglich: die erweiterte Zeile bleibt `role='assistant'` mit String-`content`, genau der Test den `get_round_bounds()` als sauberen Abschluss prüft.
...
- `create_thread(title=None, project_id, data_scope)`/`update_thread(thread_id, **fields)`/`delete_thread(thread_id)`/`list_threads(limit=50)` (Teil 2, `title` seit Teil A optional) — `delete_thread()` löscht NUR die Thread-Zeile, `messages.thread_id` bleibt unangetastet (Threads sind Etiketten, keine Behälter). `_ENTITY_REQUIRED_FIELD` (`server.py`) hat seit Teil A keinen Eintrag mehr für `threads` — Anlegen läuft ausschließlich unbenannt, Umbenennen ist ein separater Schritt (Sidebar).
...
- `get_thread_messages(thread_id)` (Teil 2, trägt seit Teil A zusätzlich die echte `messages.id` pro Eintrag) — für die Chat-Anzeige beim Thread-Wechsel: `text = display_text or _extract_text(...)`, Platzhalter (`is_placeholder_text()`) übersprungen. Die `id` ist die Grundlage fürs Verschieben (s.u.) — ohne sie ließe sich eine Nachricht nicht als Ziel für `move_round`/`move_from_here` adressieren.

**Rundenerkennung + Verschieben/Zusammenführen (Thread-Umbau Teil A, neu):** eine "Runde" beginnt bei einer `role='user'`-Zeile, deren `content` NICHT ausschließlich aus `tool_result`-Blöcken besteht, und endet bei der ersten darauffolgenden `role='assistant'`-Zeile mit reinem String-`content` (derselbe "sauberer Rundenabschluss"-Test, den `repair_dangling_turns()` schon nutzt). Da jede abgeschlossene Runde dadurch ein in sich stimmiger, mit `user` beginnender/`assistant` endender Block ist, erhält das Verschieben eines beliebigen Teils solcher Blöcke die Rollen-Abwechslung IMMER, unabhängig vom Zielort — die mathematische Grundlage für "nur ganze Runden verschieben, nie einzelne Zeilen".
- `_is_tool_result_content(content)` — Hilfsfunktion, prüft ob `content` ausschließlich aus `tool_result`-Blöcken besteht (Fortsetzung eines Tool-Loops, kein echter Rundenanfang).
- `get_round_bounds(message_id)` — findet zu einer `user`-Zeile das Ende der zugehörigen Runde (nächste saubere `assistant`-Zeile). `None` falls `message_id` selbst kein gültiger Rundenanfang ist (z.B. `tool_result`-Fortsetzung).
- `_touch_threads(*thread_ids)` — aktualisiert `threads.last_activity_at` für mehrere Threads in einem Aufruf (reines `UPDATE messages` löst das anders als `append_message()` nicht automatisch aus).
- `move_round(message_id, target_thread_id)` — "nur diese Runde": setzt `thread_id`/`project_id` NUR für die Zeilen dieser einen Runde.
- `move_from_here(message_id, target_thread_id)` — Standard "ab hier alles": setzt `thread_id`/`project_id` für ALLE Zeilen mit `thread_id = <Quell-Thread> AND id >= message_id` (Filterung nur über `thread_id`, nicht `tab_id`/`category` — ein Thread kann laut Design geräte-/tab-übergreifend fortgesetzt werden).
- `merge_threads(source_id, target_id)` — überträgt ALLE Nachrichten des Quell-Threads (`UPDATE messages SET thread_id=target_id, project_id=<Ziel-project_id>`), aktualisiert `last_activity_at` des Ziels, löscht danach den Quell-Thread. Zielreihenfolge ergibt sich automatisch aus `ORDER BY id`.
- Aufgerufen von `server.py`s `MOVE_MESSAGES`/`MERGE_THREADS`-Handlern (siehe dort für den Protokoll-Teil inkl. `THREAD_REASSIGNED`).

**Automatische Benennung unbenannter Threads (Thread-Umbau Teil B, Schritt 1, neu):**
- `count_rounds(thread_id)` — zählt echte Rundenanfänge (`role='user'`, nicht `tool_result`-förmig, reuse `_is_tool_result_content()`) — dieselbe Definition wie `get_round_bounds()`, hier nur gezählt statt einzeln aufgelöst. Grundlage für die Rundenzahl-Gates in `thread_naming.py`.
- `get_thread_title(thread_id)` — billiger PK-Lookup, Vorab-Check vor Rundenzählung/LLM-Call.
- `set_auto_title(thread_id, title)` — atomares `UPDATE threads SET title = ? WHERE id = ? AND title IS NULL`, gibt zurück ob tatsächlich geschrieben wurde (`rowcount > 0`). Der eigentliche Race-Schutz für "nie überschreiben" — läuft der Hintergrund-Call während einer parallelen manuellen Umbenennung, greift die WHERE-Bedingung nicht mehr, kein Clobbern.
- `list_named_titles(limit=300)` — für den Naming-Prompt. Bewusst NICHT `list_threads()` wiederverwendet (nach `last_activity_at DESC` sortiert, auf 50 gedeckelt, für die Sidebar gedacht) — ein seit Monaten inaktiver gleichnamiger Thread würde dort aus dem Fenster fallen und dem Namensgeber nie als Wiederverwendungs-Kandidat angeboten.
- `list_unnamed_thread_ids()` — für den einmaligen Startup-Sweep (`thread_naming.run_startup_sweep()`), alle Threads mit `title IS NULL`.
...
Reine Konstanten, keine Logik, `import protocol as P`, referenziert als `P.XXX`. 70 Konstanten (die bisherige Angabe "46" war bereits vor Thread-Umbau Teil A veraltet — mehrere Gruppen wurden seit dem Stand 2026-07-24 ergänzt, ohne die Zahl hier nachzuziehen; jetzt frisch nachgezählt, inkl. der drei `PCM_*`-Konstanten, der fünf Thread-Umbau-Teil-A-Typen und `THREAD_TITLE_UPDATED` aus Teil B, Schritt 1):

| Gruppe | Konstanten |
|---|---|
| Server → alle Clients | `STATE`, `STATUS`, `TRANSCRIPT`, `RESPONSE_START/CHUNK/DONE` (`RESPONSE_START` trägt seit Thread-Umbau Teil A `user_message_id`, `RESPONSE_DONE` trägt `assistant_message_id` — nur bei echtem Turn-Abschluss gesetzt, siehe `pipeline.py::_run_llm()`), `TOOL`, `ERROR`, `PONG` |
...
| Threads verschieben/zusammenführen (Thread-Umbau Teil A, neu) | `MOVE_MESSAGES` (→Server — `{message_id, mode:"from_here"\|"single_round", target_thread_id}`), `MOVE_MESSAGES_ACK` (→Client — `{ok, error?}`), `MERGE_THREADS` (→Server — `{source_thread_id, target_thread_id}`), `MERGE_THREADS_ACK` (→Client — `{ok, error?}`), `THREAD_REASSIGNED` (→Client — `{old_thread_id, new_thread_id}`, nur an Tabs deren aktiver Thread gerade wegfusioniert wurde) |
| Automatische Thread-Benennung (Thread-Umbau Teil B, Schritt 1, neu) | `THREAD_TITLE_UPDATED` (→ALLE Web-/Dashboard-Clients — `{thread_id, title}`, Broadcast statt Einzel-Tab-Push wie bei `THREAD_REASSIGNED`, da reine Anzeige-Aktualisierung ohne Zustandsrisiko, siehe `server.py::_broadcast_web_event()`) |
...
- `complete(system, user_text, model="claude-haiku-4-5", max_tokens=60)` (Thread-Umbau Teil B, Schritt 1, neu) — minimaler, NICHT-streamender Einzel-Call (`_get_client().messages.create()`) für billige Hintergrund-Aufgaben (erster Aufrufer: `thread_naming.py`). Kein Tool-Loop, kein Cache-Control, kein fester 24000-Token-Deckel wie `stream()` — ein einzelner kurzer Call hat keinen Nutzen von Streaming-Overhead. Gibt `(Text, usage)` zurück, `usage` läuft optional durch `compute_cost()`.
